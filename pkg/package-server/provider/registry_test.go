...
func NewFakeRegistryProvider(ctx context.Context, clientObjs []runtime.Object, k8sObjs []runtime.Object, globalNamespace string) (*RegistryProvider, error) {
	...
	op, err := queueinformer.NewOperator(opClientFake.KubernetesInterface().Discovery(), queueinformer.WithNumWorkers(1))
	if err != nil {
		return nil, err
	}
...
func TestPackageManifestChannelsDisplayOrder(t *testing.T) {
	apiPkg := &api.Package{
		Name: "etcd",
		Channels: []*api.Channel{
			{
	...
	require.Equal(t, &operators.PackageManifest{
		...
		Status: operators.PackageManifestStatus{
			...
			Channels: []operators.PackageChannel{
				...
				{
					Name:       "beta",
					CurrentCSV: "etcdoperator.v0.10.1",
					CurrentCSVDesc: func() operators.CSVDescription {
						csv := operatorsv1alpha1.ClusterServiceVersion{}
						require.NoError(t, json.Unmarshal([]byte(etcdCSVJSONv10), &csv))
						return operators.CreateCSVDescription(&csv, etcdCSVJSONv10)
					}(),
				},
			},
		},
