...
func NewRegistryProvider(crClient versioned.Interface, operator *queueinformer.Operator, wakeupInterval time.Duration, globalNamespace string) *RegistryProvider {
	p := &RegistryProvider{
		Operator: operator,
	...
	factory := externalversions.NewSharedInformerFactoryWithOptions(crClient, wakeupInterval)
	catsrcInformer := factory.Operators().V1alpha1().CatalogSources()

	// Register queue and QueueInformer
	logrus.Info("watching catalogsources")
	queueName := "catalogsources"
	sourceQueue := workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), queueName)
	sourceQueueInformer := queueinformer.NewInformer(sourceQueue, catsrcInformer.Informer(), p.syncCatalogSource, sourceHandlers, queueName, metrics.NewMetricsNil(), logrus.New())
