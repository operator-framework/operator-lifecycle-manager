...
// List lists all OperatorConditions in the indexer.
func (s *operatorConditionLister) List(selector labels.Selector) (ret []*v1.OperatorCondition, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.OperatorCondition))
	})
	return ret, err
}
...
// List lists all OperatorConditions in the indexer for a given namespace.
func (s operatorConditionNamespaceLister) List(selector labels.Selector) (ret []*v1.OperatorCondition, err error) {
...
// Get retrieves the OperatorCondition from the indexer for a given namespace and name.
func (s operatorConditionNamespaceLister) Get(name string) (*v1.OperatorCondition, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("operatorcondition"), name)
	}
	return obj.(*v1.OperatorCondition), nil
}
