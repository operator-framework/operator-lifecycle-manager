...
type CatalogSourceLister interface {
	...
	List(selector labels.Selector) (ret []*operators.CatalogSource, err error)
...
// List lists all CatalogSources in the indexer.
func (s *catalogSourceLister) List(selector labels.Selector) (ret []*operators.CatalogSource, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*operators.CatalogSource))
	})
	return ret, err
}
...
type CatalogSourceNamespaceLister interface {
	...
	Get(name string) (*operators.CatalogSource, error)
...
// List lists all CatalogSources in the indexer for a given namespace.
func (s catalogSourceNamespaceLister) List(selector labels.Selector) (ret []*operators.CatalogSource, err error) {
...
// Get retrieves the CatalogSource from the indexer for a given namespace and name.
func (s catalogSourceNamespaceLister) Get(name string) (*operators.CatalogSource, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(operators.Resource("catalogsource"), name)
	}
	return obj.(*operators.CatalogSource), nil
}
