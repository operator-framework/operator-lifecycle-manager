...
type CatalogSourceInformer interface {
	...
	Lister() internalversion.CatalogSourceLister
}
...
func NewCatalogSourceInformer(client clientsetinternalversion.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredCatalogSourceInformer(client clientsetinternalversion.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Operators().CatalogSources(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Operators().CatalogSources(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *catalogSourceInformer) defaultInformer(client clientsetinternalversion.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *catalogSourceInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&operators.CatalogSource{}, f.defaultInformer)
}

func (f *catalogSourceInformer) Lister() internalversion.CatalogSourceLister {
	return internalversion.NewCatalogSourceLister(f.Informer().GetIndexer())
}
