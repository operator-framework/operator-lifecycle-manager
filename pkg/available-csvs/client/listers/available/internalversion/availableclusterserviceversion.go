...
type AvailableClusterServiceVersionLister interface {
	...
	List(selector labels.Selector) (ret []*available.AvailableClusterServiceVersion, err error)
...
// List lists all AvailableClusterServiceVersions in the indexer.
func (s *availableClusterServiceVersionLister) List(selector labels.Selector) (ret []*available.AvailableClusterServiceVersion, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*available.AvailableClusterServiceVersion))
	})
	return ret, err
}
...
type AvailableClusterServiceVersionNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*available.AvailableClusterServiceVersion, error)
...
// List lists all AvailableClusterServiceVersions in the indexer for a given namespace.
func (s availableClusterServiceVersionNamespaceLister) List(selector labels.Selector) (ret []*available.AvailableClusterServiceVersion, err error) {
...
// Get retrieves the AvailableClusterServiceVersion from the indexer for a given namespace and name.
func (s availableClusterServiceVersionNamespaceLister) Get(name string) (*available.AvailableClusterServiceVersion, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(available.Resource("availableclusterserviceversion"), name)
	}
	return obj.(*available.AvailableClusterServiceVersion), nil
}
