...
import (
	...
	"github.com/operator-framework/operator-lifecycle-manager/pkg/available-csvs/apiserver/generic"
	generatedopenapi "github.com/operator-framework/operator-lifecycle-manager/pkg/available-csvs/client/openapi"
	"github.com/operator-framework/operator-lifecycle-manager/pkg/available-csvs/version"
)
...
func (c *Config) Complete(informers informers.SharedInformerFactory) completedConfig {
	...
	c.GenericConfig.OpenAPIConfig.Info.Title = "Available ClusterServiceVersion API server"
...
type AvailableCSVServer struct {
	*genericapiserver.GenericAPIServer
}

// New returns a new instance of MetricsServer from the given config.
func (c completedConfig) New() (*AvailableCSVServer, error) {
	genericServer, err := c.CompletedConfig.New("availablecsv-server", genericapiserver.NewEmptyDelegate()) // completion is done in Complete, no need for a second time
	if err != nil {
		return nil, err
	}
