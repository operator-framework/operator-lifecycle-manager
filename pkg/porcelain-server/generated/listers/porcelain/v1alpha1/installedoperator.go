...
type InstalledOperatorLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.InstalledOperator, err error)
...
// List lists all InstalledOperators in the indexer.
func (s *installedOperatorLister) List(selector labels.Selector) (ret []*v1alpha1.InstalledOperator, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.InstalledOperator))
	})
	return ret, err
}
...
type InstalledOperatorNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.InstalledOperator, error)
...
// List lists all InstalledOperators in the indexer for a given namespace.
func (s installedOperatorNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.InstalledOperator, err error) {
...
// Get retrieves the InstalledOperator from the indexer for a given namespace and name.
func (s installedOperatorNamespaceLister) Get(name string) (*v1alpha1.InstalledOperator, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("installedoperator"), name)
	}
	return obj.(*v1alpha1.InstalledOperator), nil
}
