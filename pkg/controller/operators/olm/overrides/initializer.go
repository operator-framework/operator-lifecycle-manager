...
func NewDeploymentInitializer(logger *logrus.Logger, lister operatorlister.OperatorLister) *DeploymentInitializer {
...
func (d *DeploymentInitializer) initialize(ownerCSV ownerutil.Owner, deployment *appsv1.Deployment) error {
	...
	// TODO(tflannag): Should we be ignoring the environment variable overrides here?
	_, volumeOverrides, volumeMountOverrides, tolerationOverrides, resourcesOverride, nodeSelectorOverride, err := d.config.GetConfigOverrides(ownerCSV)
	if err != nil {
