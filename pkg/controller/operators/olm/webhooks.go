...
func (a *Operator) installWebhook(desc v1alpha1.WebhookDescription, ca *certs.KeyPair, rotateAt time.Time, depSpec appsv1.DeploymentSpec, csv *v1alpha1.ClusterServiceVersion) (*appsv1.DeploymentSpec, error) {
	...
	case v1alpha1.InstallWebhookValidating:
		...
		existingHook, err := a.opClient.KubernetesInterface().AdmissionregistrationV1().ValidatingWebhookConfigurations().Get(desc.Name, metav1.GetOptions{})
		if err == nil {
			// Check if the only owners are this CSV or in this CSV's replacement chain
			if ownerutil.Adoptable(csv, existingHook.GetOwnerReferences()) {
				ownerutil.AddNonBlockingOwner(existingHook, csv)
			}
			...
			// Attempt an update
			if _, err := a.opClient.KubernetesInterface().AdmissionregistrationV1().ValidatingWebhookConfigurations().Update(existingHook); err != nil {
				logger.Warnf("could not update ValidatingWebhookConfiguration %s", existingHook.GetName())
				return nil, err
			}
		} else if k8serrors.IsNotFound(err) {
	...
	case v1alpha1.InstallWebhookMutating:
		...
		existingHook, err := a.opClient.KubernetesInterface().AdmissionregistrationV1().MutatingWebhookConfigurations().Get(desc.Name, metav1.GetOptions{})
		if err == nil {
			// Check if the only owners are this CSV or in this CSV's replacement chain
			if ownerutil.Adoptable(csv, existingHook.GetOwnerReferences()) {
				ownerutil.AddNonBlockingOwner(existingHook, csv)
			...
			// Attempt an update
			if _, err := a.opClient.KubernetesInterface().AdmissionregistrationV1().MutatingWebhookConfigurations().Update(existingHook); err != nil {
				logger.Warnf("could not update MutatingWebhookConfiguration %s", existingHook.GetName())
				return nil, err
			}
		} else if k8serrors.IsNotFound(err) {
