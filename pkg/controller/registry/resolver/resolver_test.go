...
func TestSolveOperators_CatsrcPrioritySorting(t *testing.T) {
	...
	//communityCatalog := registry.CatalogKey{"community-operator", namespace}
	customCatalog := registry.CatalogKey{"community", namespace}
	//highPriorityCatalog := registry.CatalogKey{"high-priority-operator", namespace}
	...
	fakeNamespacedOperatorCache := NamespacedOperatorCache{
		snapshots: map[registry.CatalogKey]*CatalogSnapshot{
			...
			registry.CatalogKey{
				...
				priority: solver.Priorities{CatalogSource: 100},
				operators: []*Operator{
