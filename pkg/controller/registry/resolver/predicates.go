...
func (c catalogPredicate) String() string {
	return fmt.Sprintf("with catalog: %v/%v", c.key.Namespace, c.key.Name)
}
...
func (g gvkPredicate) String() string {
	return fmt.Sprintf("with group: %s, version: %s, kind: %s", g.api.Group, g.api.Version, g.api.Kind)
}
...
func (opf OperatorPredicateFunc) String() string {
	return ""
}
