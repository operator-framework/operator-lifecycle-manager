...
func (ctl *SubscriptionController) installplanForSubscription(sub subscriptionv1alpha1.Subscription) (*installplanv1alpha1.Installplan, error) {
	plan, err := ctl.catalog.FetchInstallplanForAppVersion(sub.Spec.AppType, sub.Status.CurrentVersion)
	if err != nil {
		return nil, err
	}
	return &plan, ctl.installer.CreateInstallplan(sub.GetNamespace(), planl)
}
