...
func TestGlobalCatalogUpdateTriggersSubscriptionSync(t *testing.T) {
	...
	for _, ns := range namespaces.Items {
		if ns.GetName() == "openshift-operator-lifecycle-manager" {
			globalNS = "openshift-operator-lifecycle-manager"
		}
	}
	...
	// Ensure the timing
	catalogConn := fetchedUpdatedCatalog.Status.LastSync
	subUpdatedTime := subscription.Status.LastUpdated
	timeLapse := subUpdatedTime.Sub(catalogConn.Time).Seconds()
