...
var _ = Describe("Subscription", func() {
	...
	Context("with dependencies from different catsrc with priorities", func() {
		...
		Context("creating CatalogSources with different names", func() {
			...
			When("creating subscription for the main package", func() {
				...
				It("picks the correct CSVs based based on lexicographical ordering of catalog source names", func() {
					// ensure correct CSVs were picked
		...
		Context("creating CatalogSources with same name but different priority values", func() {
			var catsrcCleanup1, catsrcCleanup2 cleanupFunc

			BeforeEach(func() {

				//same dependency exists on both the catsrc of the installing operator and another catsrc with a higher priority", func() {
				packageDepRight = registry.PackageManifest{PackageName: "PackageDependent"}
				csv := newCSV(rightCSVName, testNamespace, "", semver.MustParse("0.1.0"),
					[]apiextensions.CustomResourceDefinition{crd}, nil, nil)
				packageDepRight.DefaultChannelName = alphaChannel
				...
				catsrcDepWrong, catsrcCleanup2 = createInternalCatalogSource(kubeClient, crClient, genName("catsrc"), testNamespace,
					[]registry.PackageManifest{packageDepWrong}, []apiextensions.CustomResourceDefinition{crd}, csvsWrong)
				updateCatSrcPriority(crClient, testNamespace, catsrcDepWrong, 100)
		...
		Context("creating CatalogSources (the same dependency exists) with different priority value", func() {
			var catsrcCleanup1, catsrcCleanup2, catsrcCleanup3 cleanupFunc
		...
		Context("creating CatalogSources (the same dependency exists) with different namespaces", func() {
			var catsrcCleanup1, catsrcCleanup2, catsrcCleanup3 cleanupFunc
			...
			BeforeEach(func() {
				...
				catsrcDepWrong, catsrcCleanup3 = createInternalCatalogSource(kubeClient, crClient, genName("catsrc"), dNS.GetName(),
					[]registry.PackageManifest{packageDepWrong}, []apiextensions.CustomResourceDefinition{crd}, csvsWrong)
				...
				_, err = fetchCatalogSourceOnStatus(crClient, catsrcDepRight.GetName(), testNamespace, catalogSourceRegistryPodSynced)
				Expect(err).ToNot(HaveOccurred())
				_, err = fetchCatalogSourceOnStatus(crClient, catsrcDepWrong.GetName(), dNS.GetName(), catalogSourceRegistryPodSynced)
				Expect(err).ToNot(HaveOccurred())
			})
			AfterEach(func() {
				if catsrcCleanup1 != nil {
			...
			When("creating subscription for the main package", func() {
				...
				It("chooses the dependent package from the same catsrc as the installing operator based on lexicographical namespace ordering", func() {
					// ensure correct CSVs were picked
					Eventually(func() ([]string, error) {
						ip, err := crClient.OperatorsV1alpha1().InstallPlans(testNamespace).Get(context.TODO(), subscription.Status.InstallPlanRef.Name, metav1.GetOptions{})
						if err != nil {
