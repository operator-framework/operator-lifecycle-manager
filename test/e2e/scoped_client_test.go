...
func TestScopedClient(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Error(t, errGot)
			tt.assertFunc(errGot)
			gvr := schema.GroupVersionResource{Group: "g", Version: "version", Resource: "kind"}

			_, errGot = dynamicClientGot.Resource(gvr).Get(genName("does-not-exist-"), metav1.GetOptions{})
			require.Error(t, errGot)
			tt.assertFunc(errGot)
		})
	}
